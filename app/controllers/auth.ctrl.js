  ...
  User.findOne({ 'profile.email': email })
    .then( (existingUser) => {
      ...
      if (existingUser) {
        ...
        const avatarUrl = 'https://raw.githubusercontent.com/rifkegribenes/picnic/master/client/public/img/picnic_icon.png';
        const user = new User({
          local: { email, password },
        ...
        user.save((err, user) => {
          ...
          const subject = "Picnic | Email Verification Required";
...
const sendPWResetEmail = (params) => {
    ...
    const subject = 'Picnic | Password Reset Request';
  ...
  User.findOne(target)
    ...
    .then(user => {
      ...
      user.save((err) => {
        ...
        const subject = "Picnic | Password Changed";
