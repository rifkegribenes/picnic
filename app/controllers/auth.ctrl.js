  ...
  User.findOne({ 'profile.email': email })
    .then( (existingUser) => {
      ...
      if (existingUser) {
        ...
        const avatarUrl = 'https://raw.githubusercontent.com/rifkegribenes/parkbuilder/master/client/public/img/parkbuilder_icon.png';
        const user = new User({
          local: { email, password },
        ...
        user.save((err, user) => {
          ...
          const subject = "parkbuilder | Email Verification Required";
...
const sendPWResetEmail = (params) => {
    ...
    const subject = 'parkbuilder | Password Reset Request';
  ...
  User.findOne(target)
    ...
    .then(user => {
      ...
      user.save((err) => {
        ...
        const subject = "parkbuilder | Password Changed";
